...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
    ...
    Quaternion<float> qt, qtBar;
    qt = qt.FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
    qtBar = qt.IntegrateBodyRate(gyro, dtIMU);
    float predictedRoll = atan2f((2.f * (qtBar[0] * qtBar[1] + qtBar[2] * qtBar[3])), 1.f - 2.f * (pow(qtBar[1], 2) + pow(qtBar[2], 2)));
    float predictedPitch = asin(2.f * (qtBar[0] * qtBar[2] - qtBar[1] * qtBar[3]));
    ekfState(6) = atan2f(2.f * (qtBar[0] * qtBar[3] + qtBar[1] * qtBar[2]), 1.f - 2.f * (pow(qtBar[2], 2) + pow(qtBar[3], 2)));
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(5) = curState(5) + (inertialAccel.z - 9.81f) * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  for (int i = 0; i < QUAD_EKF_NUM_STATES; i++) {
      for (int j = 0; j < QUAD_EKF_NUM_STATES;j++) {
          if (i == j) {
              gPrime(i, j) = 1;
          }
          else {
